...
namespace _CurriculumManagerSystem.Views.Shared.Components.Pdf_Clopphoc
{
    //[ViewComponent]
    public class Pdf_Clopphoc : ViewComponent
    {
        private readonly acomptec_lvthainhanContext _context;

        public Pdf_Clopphoc(acomptec_lvthainhanContext context)
        {
            _context = context;
        }
        ...
        {  
            var listPphoc = _context.Phuongphap_Hocs;
            ViewBag.listPphoc = listPphoc.ToList();
              
             var acomptec_lvthainhanContext = _context.CLOPhuongphaphocs.Include(c => c.Phuongphap_hoc).Include(c => c.Chuandaura_monhoc).Where(c=> c.Chuandaura_monhoc.mahp == id).OrderBy(c=> c.Chuandaura_monhoc.chisocio);
